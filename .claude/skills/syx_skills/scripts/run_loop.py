...
在循环中结合 run_eval.py 和 improve_description.py，跟踪历史记录
并返回找到的最佳描述。支持训练/测试集划分以防止过拟合。
"""
...
def split_eval_set(eval_set: list[dict], holdout: float, seed: int = 42) -> tuple[list[dict], list[dict]]:
    """将评估集划分为训练集和测试集，按 should_trigger 分层。"""
    random.seed(seed)
...
def run_loop(
    ...
    # 如果 holdout > 0，划分为训练集/测试集
    if holdout > 0:
        train_set, test_set = split_eval_set(eval_set, holdout)
        if verbose:
            print(f"划分: {len(train_set)} 训练, {len(test_set)} 测试 (holdout={holdout})", file=sys.stderr)
    else:
        train_set = eval_set
        test_set = []
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if verbose:
            def print_eval_stats(label, results, elapsed):
                ...
                print(f"{label}: {tp+tn}/{total} 正确, precision={precision:.0%} recall={recall:.0%} accuracy={accuracy:.0%} ({elapsed:.1f}秒)", file=sys.stderr)
                for r in results:
                    status = "通过" if r["pass"] else "失败"
                    rate_str = f"{r['triggers']}/{r['runs']}"
                    print(f"  [{status}] 率={rate_str} 期望={r['should_trigger']}: {r['query'][:60]}", file=sys.stderr)
        ...
        if train_summary["failed"] == 0:
            exit_reason = f"all_passed (迭代 {iteration})"
            if verbose:
                print(f"\n所有训练查询在第 {iteration} 次迭代时通过！", file=sys.stderr)
            break
        ...
        if verbose:
            print(f"建议 ({improve_elapsed:.1f}秒): {new_description}", file=sys.stderr)

        current_description = new_description

    # 按测试分数找到最佳迭代（如果没有测试集则按训练分数）
    if test_set:
    ...
    if verbose:
        ...
        print(f"最佳分数: {best_score} (迭代 {best['iteration']})", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--report", default="auto", help="在此路径生成 HTML 报告（默认: 'auto' 为临时文件, 'none' 为禁用）")
    parser.add_argument("--results-dir", default=None, help="将所有输出（results.json、report.html、log.txt）保存到此处带时间戳的子目录中")
    ...
    # 确定输出目录（在 run_loop 之前创建，以便日志可以写入）
    if args.results_dir:
        timestamp = time.strftime("%Y-%m-%d_%H%M%S")
