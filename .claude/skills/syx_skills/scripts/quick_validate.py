...
def validate_skill(skill_path):
    ...
    if not isinstance(name, str):
        return False, f"名称必须是字符串，实际类型为 {type(name).__name__}"
    name = name.strip()
    if name:
    ...
    if not isinstance(description, str):
        return False, f"描述必须是字符串，实际类型为 {type(description).__name__}"
    description = description.strip()
    if description:
        # 检查尖括号
        if '<' in description or '>' in description:
            return False, "描述不能包含尖括号（< 或 >）"
        # 检查描述长度（按规范最多 1024 个字符）
        if len(description) > 1024:
            return False, f"描述过长（{len(description)} 个字符）。最大为 1024 个字符。"

    # 验证 compatibility 字段（如果存在，可选）
    compatibility = frontmatter.get('compatibility', '')
    if compatibility:
        if not isinstance(compatibility, str):
            return False, f"兼容性必须是字符串，实际类型为 {type(compatibility).__name__}"
        if len(compatibility) > 500:
            return False, f"兼容性信息过长（{len(compatibility)} 个字符）。最大为 500 个字符。"
