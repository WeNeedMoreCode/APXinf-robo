...
| 37 | geb_model_build 返回 **rc=-8** | **rc=-8 = aclmdlLoadFromMem 失败**（不是编译上限）——"model built: N bytes" 已打印、装载阶段挂：最常见是烤桶时芯片被占（大 OM 3.7GB 装不下）；曾两次被误读为"编译规模上限"（2026-09-22 定案：depth18 全深度编译+落盘在干净芯片全通） | 烤桶前确认目标芯片空闲（npu-smi）；rc=-8 时 grep 日志 "model built"（编译是否成功）与 "aclmdlLoadFromMem failed"（装载失败实锤）；换干净芯片重试即可 |
