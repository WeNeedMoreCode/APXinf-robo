...
| 35 | ReduceSumD 编译 rc=-7（attr 名 "axis"）；TileD 输入 [rows,1]+multiples=[1,w] 被 infershape 推成 [1,rows*w]；RealDiv 广播 [m,w]÷[m,1] 编译拒（mul E80013 同族） | 归约族 attr 名是 **"axes"**（list）非 "axis"；除法/乘法算子无广播 kernel | ReduceSumD 用 "axes"；行广播用 #39 的 dim0 复制 + TransposeD 桥（⚠ 2026-09-22 追记：曾推荐的"1-D Tile 域 + 双 Reshape 桥"形态有 #39 的散点腐蚀，勿用） |
...
| 37 | 大图（算子数近翻倍）编译 rc=-8，小深度同图全通 | **2026-09-22 定案：伪编译上限**——stdout 日志（工具箱 #8）显示 18 层图编译+task 生成全部成功，真凶是后续 eager 参考构建 panic（binds 索引错位 #29 族）被误读为编译失败；另 GEB_SAVE 若在早退分支之后会漏存 OM | 大图失败先 `ASCEND_SLOG_PRINT_TO_STDOUT=1` 看 "OptimizeSubgraph success/GenerateTask" 有没有走到；GEB_SAVE 放早退分支内或 build 后立即执行 |
| 38 | f32 desc 的 ReduceSumD/ReduceSum 输出 ≈ 65504 附近（≈ host 的 8%）；opp 注册表 json 明明有 d=1（fp32）simplifiedKey | **310P tbe 归约算子的 f32 是假支持**——FE 混精度路径把 f32 图回退成 f16 kernel（内部 f16 累加），行和 >65504 饱和；Cast 的 f32 倒是真语义（f16→f32 直出逐位）。与 #arm32 的 AddRmsNorm 假 f32 同族 | 方差/大数和的 fp32 语义改走 **MatMulV2（f16 in/out）**：cube 引擎内部 fp32 累加（实测 0.05% = f16 量化级）。预缩放防单元素平方溢出 f16：xs=x·s（s=1/32）→ sq=xs² → ssum=mm(sq, ones [1,w] Const) → rsqrt(ssum)·(s·√w) 折回 |
| 39 | TileD 做"每元素连续重复"方向广播（rank-1 [rows]+multiples=[w]，或 [1,rows]+multiples=[1,w] flat 形态）：编译运行全正常，但输出**散点腐蚀 ~8%**（两种 rank 锚定逐位同错——同一 kernel）；dim0 整行复制方向（[1,w]+multiples=[rows,1]）生产验证干净 | 310P f16 的连续平铺 kernel 自身缺陷（编译/执行零报错，纯数值坑） | 行广播（per-row 标量铺满 [rows,w]）用 **dim0 复制 + 转置桥**：[rows,1]→Reshape [1,rows]→TileD multiples=[w,1]→[w,rows]→TransposeD(perm=[1,0])→[rows,w]（实测 0.08% 干净，2026-09-22 n32v t11/t13/t14 定罪） |
