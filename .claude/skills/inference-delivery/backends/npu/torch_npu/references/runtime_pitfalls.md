...
## 12. Bench 纪律：warmup 均值陷阱 + 同机自对拍定噪声底

两个正交的测量陷阱，都来自 π0.5 VLA 移植实战（2026-09）。

### 12.1 冷启动编译摊进均值

**现象**：闭环/bench 报的平均单步延迟是 bench 稳态值的数倍（实测 1378ms vs 375ms，
3.7 倍），但逐次打印分布又发现绝大多数调用都在 375ms 附近。

**根因**：torchair 图编译是**一次性成本**（大模型 ~100s），发生在首次真实调用。
任何把首调用摊进平均的测量（episode 内所有调用的均值、小样本 bench）都被它污染：
104 次调用的均值里，100s ÷ 104 ≈ 960ms 是编译摊销。

**解法**：评测/bench 前**显式 warmup**——用与真实调用同 shape 的合成输入（如 zeros
观测）先跑一次 `select_action`/forward，让编译发生在计时之外。同时**看分布不看均值**
（P50/P90），分布能立刻暴露"一次离群 + 稳态正常"的模式，均值会把它藏起来。

### 12.2 同机自对拍 = 可重复性上限

**现象**：对拍/复现实验中，同输入两次推理的输出永远差一点（~2⁻¹⁰，1 个 fp16 ulp），
纠结是否没对齐。

**根因**：aclnn 算子调度**非确定**——同芯片同输入同权重，两次执行也可能选不同调度
路径，fp16 下产生 1 ulp 级差异。这不是 bug，bitwise 等价在该硬件上不可达。

**解法（操作化判据）**：对拍前先做**同机自对拍**——同输入连跑两次，diff 即该硬件的
可重复性底噪。之后任何跨路径对拍：diff 与底噪同量级 → 等价；显著大于底噪（实测跨
路径正常差异可与"异噪声"级别的 0.25 分庭）→ 有真分岔。π0.5 noise 注入验证即用此
判据：同 noise 两次 2⁻¹⁰（底噪），异 noise 0.25（真分岔）。
