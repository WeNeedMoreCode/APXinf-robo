...
- **阶段 2 状态**（2026-09-19）：M2 完成（真 checkpoint 全链路）→ 性能阶段 906.9→679.8ms（rope 平铺 + scratch 池）→ 验收线判定：ACLGraph matmul task ~475µs 调度税为架构级（decisions/002）→ **GE 原生 OM POC 完成**（零调度税 + 大 m tiling −18%，skill `ge-offline-om`）→ **C1 完成**（ge_builder FFI + 单层 GE 化对拍 0.00000、1.76× vs eager）→ **C2 三段 OM 全量落地**（bench ~2182ms 距 378ms 线 5.8×）→ **C2 攻坚第一轮**：busy 87ms vs 墙钟 700ms 定律、排除 PFA/unknown-shape、eager 竞态修复（vision 0.1%）→ **C2 攻坚第二轮：根因捕获 = SliceD 列切视图的运行时 MemcopyAsync 物化（每视图 ~5.5ms host 停顿）**；三段修复（GEB_QKV3 独立投影 / GEB_ROPEFLAT flat 行交换 / GEB_ATTN=manual）→ **全模型 2182→~431ms（5.06×），距 378ms 验收线 14%**（vision 68.06/0.1%、prefix 187.75/25.1%⚠、flow 17.51/步/1.6%）。陷阱 #12-29 回填 skill；skill `ascend-msprof`（最小采集纪律 + 导出失败 sqlite 恢复）
