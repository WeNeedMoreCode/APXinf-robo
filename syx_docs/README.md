...
- **阶段 2 状态**（2026-09-20）：M2 完成（真 checkpoint 全链路）→ 性能阶段 906.9→679.8ms（rope 平铺 + scratch 池）→ 验收线判定：ACLGraph matmul task ~475µs 调度税为架构级（decisions/002）→ **GE 原生 OM POC 完成**（零调度税 + 大 m tiling −18%，skill `ge-offline-om`）→ **C1 完成**（ge_builder FFI + 单层 GE 化对拍 0.00000、1.76× vs eager）→ **C2 三段 OM 全量落地**（bench ~2182ms 距 378ms 线 5.8×）→ **C2 攻坚第一轮**：busy 87ms vs 墙钟 700ms 定律、排除 PFA/unknown-shape、eager 竞态修复（vision 0.1%）→ **C2 攻坚第二轮：根因捕获 = SliceD 列切视图的运行时 MemcopyAsync 物化**；三段修复（GEB_QKV3/GEB_ROPEFLAT/GEB_ATTN=manual）→ 全模型 ~431ms（5.06×）→ **C2 收尾（2026-09-20）：prefix 漂移销案（fp16 大 K 累加序，M3 e2e 判）+ NZ 权重税根因（每执行 ND→FRACTAL_NZ TransData，54% flow 时长；torch_npu 同源税）+ GEB_WCONST 权重 Const 入图（编译期折叠，单算 −25% 实证）→ 全模型 2182→263.3ms = 378ms 验收线 0.70×（8.3× 累计，比 torch_npu 快 31%），C 路线达标进 M3**。vision 55.89/0.1%、prefix 128.44/25.1%、flow 7.90ms/步/1.6%；OM {vision,prefix,flow}_r3.om（per-checkpoint）。陷阱 #12-30 回填 skill；skill `ascend-msprof`（最小采集纪律 + 导出失败 sqlite 恢复）→ **M3 起跑（2026-09-20，281d703）：真 checkpoint 权重接入三段 OM（GEB_CKPT，复用 Pi05Weights host 解析 + bf16→f16 + Const 路径）→ {vision,prefix,flow}_real.om 三件套，bench 与合成打平（55.86/124.61/7.89ms，全模型 ~259ms）真权重零延迟代价；prefix 真权重漂移 16.8→100%（d2-18，layer0 仍逐位一致、次线性平滑 = outlier 放大累加序机制，e2e 判）**。M3 剩余：三段接入真实推理链（真嵌入/style 条件/pk-pv 直连）→ e2e bench + LIBERO 对标（9/10 基线）
