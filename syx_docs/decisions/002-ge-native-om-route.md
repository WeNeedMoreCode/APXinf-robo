...
**执行进展（C2 收尾，2026-09-20）——验收线达标（263ms = 0.70×），进 M3**：prefix 漂移深度扫描销案（平滑次线性 + layer0 逐位一致 → fp16 大 K 累加序差，probe 侧结案）；flow 每层 0.975ms 中 **~0.53ms 是 NZ 权重税**——GE 对每个 ND Data mm 权重每执行插设备侧 ND→FRACTAL_NZ TransData（torch_npu 397ms 里 TransData 100ms 的同源税，TorchAir 靠权重入图逃掉）；单算裁决实证 **Const 权重编译期折叠**（GEB_OPTEST=wgt：0.3945→0.2945 ms/mm，OM 烤入权重）；**GEB_WCONST 三段推广**（eager 索引镜像设计，parity 不变）→ vision 55.89 / prefix 128.44 / flow 7.90ms/步 → **全模型 263.3ms（累计 8.3×）= 378ms 线 0.70×，比 torch_npu 基线快 31%**。**约束更新：OM 变 per-checkpoint（权重烤入），缓存 key 须含权重版本——与 TorchAir 同模型，工程代价可承受（构建期一次性 host NZ 折叠 + OM ~5.2GB）。剩余优化空间：prefix MLP 算力下限（~120ms busy）与 INT8/量化领域；nz desc 直入（OM 保持 checkpoint 无关的替代）在 310P 未走通，记开放项。**
