...
**执行进展（C2 攻坚第一轮，2026-09-19）**：msprof 三轮定律修正了上述定性——**胶水 kernel 不是主因**：vision OM ~700ms 中设备 busy 仅 ~87ms（胶水贡献 ~30ms），**~600ms 是 81 次/迭代的 EVENT→~20ms→MemcopyAsync 空隙**（host 参与的运行时调度路径）。排除 PFA host 回调（手工 attention 替换后仍 700ms）与 unknown-shape 拆分（Const 折叠 OM 221→16MB 仍 700ms；flow 带 unknown 标记 + PFA 仅 70.9ms 反例）。头号嫌疑收敛到**图输出数量**（vision 111 个 LN aux 输出 vs flow 1 个），最小 A/B 验证待下轮。核心算子链进一步强化：bmm 20µs/次（PFA 323µs 的 1/16）、手工 attention 全深对拍 0.1%。**决策含义不变但验证点更新：若图输出/调度空隙可消除则路线成立（busy 87ms ≈ 验收线内），否则需评估 310P 静态 OM 执行器的调度机制上限。**
