...
| `cargo run --example ge_model_probe --features ascend -p apxinf-model` | **C2 三段 OM 全量探针**（GEB_SEG=vision\|prefix\|flow / GEB_DEPTH / GEB_TOKENS / GEB_BENCH / GEB_SAVE=**全路径文件名**（传目录会 -2）GEB_LOAD=路径——OM 缓存在 /data/apxinf/om_cache/（**当前版 {vision,prefix,flow}_real.om = 真 checkpoint 权重烤入版**（GEB_CKPT 接入，bench 与 r3 打平 55.86/124.61/7.89ms；r3 = 合成权重版）/ GEB_OPTEST=... 单算/组合/数值验证矩阵（**wgt = NZ 权重税裁决**：nd/cst/nz 三形态 mm / **oproj = 真数据 o_proj 单算裁决**（golden m0_vis × L0 真权重，cst/nd vs eager，2026-09-21 实证逐位 0.000%）/ **arm = 真数据 AddRmsNorm 单算**（golden res0 输入；rc=-3 已结案 2026-09-21：addrms 的 y 绑图输出会触发 GE 自动补绑悬空 rstd/x_out → 模型输出数 > 声明 → run rc=-3；输出一律按 num_outputs() introspection 分配）/ **attn = 最小图对拍全家**（2026-09-21 定罪复现器：真 x0_vis 走 [attention→o_proj→MLP→L1 k]；`GEB_ATTN_MIN=1` 复现 11.041%、`GEB_ATTN_L1=mm` 去 addrms 0.036% 干净对照——[在图中间量→AddRmsNorm] 污染；变体 GEB_ATTN_FAKE/RES_GOLDEN/SWAPADD/MLP_PAD/L1=bar|swap|rshp|mbar|norm）/ **wmm~wmm5 = 单算与组合矩阵**（gate 宽N/down 大K/MLP 组合阶梯/h1 作 Data 输入/addrms 输入来源判别）——attn/wmm 系都须配 GEB_E2E_GOLDEN=frame0_v3 + GEB_CKPT（oproj））/ GEB_TRACE 中间量打印（含 head）/ GEB_DBG=1 层 0 各级观察 / GEB_NO_AUX=1 跳过 LN 辅输出（数值坏，隔离实验用）/ **GEB_ATTN=manual + GEB_QKV3=1 + GEB_ROPEFLAT=1 + GEB_WCONST=1 四件套 = 当前最优组合**（全模型 263ms = 378 线 0.70×；WCONST = mm 权重 Const 入图编译期折叠 NZ 转换，OM 变 per-checkpoint；rope 表已改 f16 量化 inv_freq——torch golden 同源）/ **GEB_CKPT=<dir> 真 checkpoint 权重**（Pi05Weights host 解析 + bf16→f16 + Const 路径，~61s 加载；⚠ GEB_LOAD 真 OM 必须配 GEB_CKPT）/ **GEB_ROUNDS+GEB_PER bench 规模参数化——假设检验用 5+3，十几秒出数**；⚠ parity ≥20% 段必须带 GEB_BENCH，否则静默 exit(1)）/ **GEB_SEG=e2e 三段链式 e2e**（M3：x0 真嵌入/styles 真值/pk-pv 直连/10 步换绑；须配四件套+GEB_CKPT，OM 从 GEB_OM_DIR 读 *_real.om；GEB_E2E_GOLDEN=<safetensors> 对拍——**golden v3 中间量逐段 BISECT**（键 vision_out/x0/x0_vis/m0/h1/m0_vis/h1_vis/res0/kvk_l{0..17}/step0_x1，**golden_gen_v3.py 产 frame0_v3.safetensors = 712 空视图剔除语义、自洽 0.03-0.12%**；v2 frame0 的 replay 键是 968 全开旧语义、与 infer 键混世界——已废弃，教训见 summary 2026-09-21）/ **bisect 隔离旋钮（2026-09-20 晚）**：GEB_DEPTH_VISION/GEB_DEPTH_PREFIX 分段深度、GEB_ATTN_PREFIX 分段 attention、GEB_E2E_STOP=prefix 截断、GEB_LAYER_OFFSET 权重层偏移（配 GEB_E2E_X0_KEY=h1 直拍 kvk_l{i} 公式）、GEB_DBG_MID/GEB_DBG_FULL 图内调试输出（⚠ 槽值可被图内复用覆写，按值判读）、GEB_E2E_DUMP_MID=<dir> 槽位落盘、GEB_OUT_PAD 用户 buffer 占位取证 / **GEB_E2E_PK_GOLD=1 pk/pv 替入 golden kvk/kvv、GEB_E2E_STYLE_GOLD=1 cond 替入 golden cond_s{step}（须配 golden v3b = frame0_v3b.safetensors，含 cond_s{0..9}[1024]；2026-09-21 step0 残差 bisect 用——两腿均不动 ⇒ 残差 = flow 段 fp16 vs torch 混合精度执行语义差，结案见 summary 2026-09-21_m3-flow-step0-residual-bisect）** / **GEB_E2E_BENCH=<N> 三段稳态 rerun bench（vision/prefix 在段内、flow 为调用级循环；styles 走预计算缓存 = 生产口径；2026-09-21 实测全口径 310.3ms/调用 = torch 基线 0.82×，见 summary 2026-09-21_m3-e2e-steady-bench）** / **GEB_E2E_REPLAY=<safetensors> 离线轨迹回放（2026-09-22：真 env rollout 录制帧 patches/token_ids/noise/nact 三段逐帧换绑对拍 torch normalized_actions——须 GEB_TOKENS=等长、GEB_OM_DIR=对应长 OM〔真实 token 长度随 state 位数浮动，replay_filter.py 过滤 + bake_one.sh 按长重烤〕；工具链 /data/apxinf/replay/{record_rollout,replay_filter,run_replay,bake_one}，32 帧 rel P50=194% 紧簇，见 summary 2026-09-22_m3-offline-replay-closed-loop）** / **GEB_E2E_SERVE=<spool 目录> 常驻服务模式（2026-09-22 晚：三段机械下沉 E2eStage，文件轮询 req/resp——单进程单桶〔GEB_TOKENS=L + GEB_OM_DIR=tl{L}〕，多桶 = supervisor 每桶一进程；协议/坑见 summary 2026-09-22_m3-closed-loop-eval。闭环工具链 `/data/apxinf/serve/`〔本地镜像 syx_docs/dev_logs/〕：supervisor.sh〔桶生命周期 + 死桶守护 respawn + lazy bake，`SERVE_CHIPS="6 4 7 5" nohup` 起在 rust 容器〕、run_ge_eval.sh、norm_f16_probe.py〔RMSNorm fp32 因果对照〕、smoke.py、parse_eval.py；OM 桶 /data/apxinf/om_cache/tl{138..148}〕** / **GEB_NORM32=1 GemmaRMSNorm fp32 方差语义组合图 v2（2026-09-22 深夜定稿：全 f16 域 + mm 立方体 fp32 累加——xs=x·s(s=1/32)→sq→mm(sq,onesᵀ Const)→rsqrt·k(k=s·√w)→[1,rows]TileD[multiples=w,1]→TransposeD(1,0) 广播→x·invb·γ；⚠ 310P tbe 的 ReduceSumD/ReduceSum f32 desc 是假支持〔FE 混精度回退 f16 累加、行和 65504 饱和〕、TileD 连续平铺方向 kernel 散点腐蚀 ~8%〔skill #38/#39〕——v1 的 Cast/Reduce/RealDiv 路线已弃用；常量全 Const 折叠无额外模型输入；单点数值 n32v 全链 0.13%；**GEB_OPTEST=n32v〔须配 GEB_NORM32=1〕= 14 级 tap 数值矩阵**〔t1 Cast 对/t3 全链/t4-t7 归约死路档案/t8 mm 路线/t9 Cast 直出/t10-t14 广播二分〕；parity 自动跳过但 GEB_SAVE 在早退分支内落盘；⚠ **rc=-8 真身 = aclmdlLoadFromMem 失败**（编译成功装载失败——烤桶时芯片被占即触发，非编译上限）；NORM32 烤桶工具 /data/apxinf/replay/bake_one_n32.sh〔本地镜像 syx_docs/dev_logs/〕→ tl<L>n32 桶〔L=200/138-145/147 已有〕；serve 系统 /data/apxinf/serve/supervisor_n32.sh〔bake/OM 目录/env 三处 n32 化，控制接口同原版；本地镜像 syx_docs/dev_logs/serve_supervisor_n32.sh〕；行为判决：task0 闭环仍 520 打满——NORM32 不修复行为，见 summary/2026-09-22_m3-norm32-matmul-route）** / **GEB_PREFIX_DROP_EMPTY=256 空视图剔除（2026-09-21 根因修复：9/10 语义 empty_camera 走 missing 路径 pad=0 被遮蔽+位置塌缩，数学等价于 x0 直接剔除该 256 行 968→712；prefix/flow 两段都要设，golden 须配 v3；OM 缓存 /data/apxinf/om_cache/t712（净图）与 t712dbg（DBG_FULL tap 图））**）） |
| `GEB_OPT_<key>=<val>` / `GEB_INIT_OPT_<key>=<val>` | ge_builder 的 build 级 / init 级编译选项直通（env 名带点必须用 `env "GEB_OPT_ge.xxx=..."` 包引号，不能裸赋值）。⚠ 选项归属（2026-09-21 定，权威索引=开源 GE 仓 `compiler/api/aclgrph/ge_ir_build.cc` CheckGlobalOptions + `api/atc/main_impl.cc`）：**`ge.*` 前缀名是 init 级**（socVersion/fusionSwitchFile/bufferOptimize/disableReuseMemory…），**build 级只收 atc 短名**（input_shape/input_format…）——传错层 rc=-7/-1 与"选项不存在"同症状；合法值如 bufferOptimize=`off_optimize`（非 off）、disableReuseMemory="0"/"1"。**`GEB_INIT_OPT_ge.fusionSwitchFile=/data/apxinf/fusion_off_inplace.json` = addrms 污染修复（2026-09-21，M3）**：关 InplaceAddRmsNormFusionPass（原地融合别名写回污染在图中间量；JSON 镜像 syx_docs/dev_logs/fusion_off_inplace.json）——**烤图/运行必须全程携带**（融合状态烤在 OM 里，当前生产缓存 = /data/apxinf/om_cache/t712fix，prefix 0.8%/flow 4.4% 且更快）。取证利器：`DUMP_GE_GRAPH=3 DUMP_GRAPH_PATH=<dir>` 逐 pass 图 dump |
