...
| `cargo run --example ge_model_probe --features ascend -p apxinf-model` | **C2 三段 OM 全量探针**（GEB_SEG=vision\|prefix\|flow / GEB_DEPTH / GEB_TOKENS / GEB_BENCH / GEB_SAVE=路径 GEB_LOAD=路径——OM 缓存在 /data/apxinf/om_cache/（r2 修复版 {vision,prefix,flow}_r2.om）/ GEB_OPTEST=... 单算/组合/数值验证矩阵 / GEB_TRACE 中间量打印（含 head）/ GEB_DBG=1 层 0 各级观察 / GEB_NO_AUX=1 跳过 LN 辅输出（数值坏，隔离实验用）/ **GEB_ATTN=manual + GEB_QKV3=1 + GEB_ROPEFLAT=1 三件套 = 攻坚第二轮修复组合**（切片视图物化根因，全模型 ~431ms；见 summary round2）/ **GEB_ROUNDS+GEB_PER bench 规模参数化——假设检验用 5+3，十几秒出数**） |
