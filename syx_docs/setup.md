...
| `cargo run --example ge_model_probe --features ascend -p apxinf-model` | **C2 三段 OM 全量探针**（GEB_SEG=vision\|prefix\|flow / GEB_DEPTH / GEB_TOKENS / GEB_BENCH / GEB_SAVE=路径 GEB_LOAD=路径——OM 缓存在 /data/apxinf/om_cache/（**r3 当前版 {vision,prefix,flow}_r3.om = WCONST 权重烤入版，5.2GB，per-checkpoint**）/ GEB_OPTEST=... 单算/组合/数值验证矩阵（**wgt = NZ 权重税裁决**：nd/cst/nz 三形态 mm）/ GEB_TRACE 中间量打印（含 head）/ GEB_DBG=1 层 0 各级观察 / GEB_NO_AUX=1 跳过 LN 辅输出（数值坏，隔离实验用）/ **GEB_ATTN=manual + GEB_QKV3=1 + GEB_ROPEFLAT=1 + GEB_WCONST=1 四件套 = 当前最优组合**（全模型 263ms = 378 线 0.70×；WCONST = mm 权重 Const 入图编译期折叠 NZ 转换，OM 变 per-checkpoint）/ **GEB_ROUNDS+GEB_PER bench 规模参数化——假设检验用 5+3，十几秒出数**） |
