...
- [ ] **下一章：PI0.5 executor（M2 运行半 + M3 的主体）——结构侦察完成（含一次认知修正，2026-09-17）**：
  ...
  - **镜像施工图（2026-09-17 终版，零探索成本可开工）**——依赖链 `vla_runtime(1013) → bf16_runtime(690) → bf16_executor(228) → kernels`，全部新增文件不动 cuda：
    ...
    - **⚠ C 阶段核心卡点——大矩阵 matmul 在 310P3 的可行路径（2026-09-18 排查现场）**：
      ...
      - **下一轮第一优先实验**（按嫌疑排序）：① **mat2 转置 stride 假设**——torch 的 linear 权重是 [out,in]，torch_npu 调 aclnnMatmul 时 mat2 大概率以 [N,K] shape + 转置 stride（stride=[1,K]）传入；我们传的是 [K,N] row-major——先试 `fp16_nd_transposed` 描述符（shape [2560,2048]、stride [1,2048]、同一块内存）直接喂现有 aclnnMatmul；② 开 CANN 日志在 8.5.1 容器跑 `torch.randn(8,2048)@torch.randn(2048,2560)` fp16 npu，看 kernel 名与参数 dump（被中断的实验，重跑即得 ground truth）；③ cubeMathType 0/2 变体
