...
- [ ] **下一章：PI0.5 executor（M2 运行半 + M3 的主体）——结构侦察完成（含一次认知修正，2026-09-17）**：
  ...
  - **镜像施工图（2026-09-17 终版，零探索成本可开工）**——依赖链 `vla_runtime(1013) → bf16_runtime(690) → bf16_executor(228) → kernels`，全部新增文件不动 cuda：
    ...
    - **⚠ C 阶段核心卡点——大矩阵 matmul 在 310P3 的可行路径（2026-09-18 排查现场）**：
      ...
      - **下一轮实验**（从这继续）：① smoke 前向逐步瘦身成 probe 形态二分第二层差异（候选：from_host 权重上传 / zeros_like 缓存 / apply_rope_rows 的 pos-gather / PFA / kv_bias 的 d2h-h2d 往返）；② 8.5.1 容器 torch_npu 同 shape 开日志抓 kernel 参数 dump 对照 desc 细节；③ 检查 AddRmsNorm 的输出 desc（xOut/rstd）是否污染源，试减输出
