  ...
  - **镜像施工图（2026-09-17 终版，零探索成本可开工）**——依赖链 `vla_runtime(1013) → bf16_runtime(690) → bf16_executor(228) → kernels`，全部新增文件不动 cuda：
    ...
    - **⚠ C 阶段核心卡点——大矩阵 matmul 在 310P3 的可行路径（2026-09-18 排查现场）**：
      ...
      - **✅ 性能阶段推进（2026-09-19 凌晨）：全深度 906.9 → 680.8ms**。三项落地：
        ...
        - **C 路线 POC 进行中（2026-09-19，1ed06e2，未完成）**：目标=GE 图内 16 个 MatMulV2 串联编译成 OM，实测 task 调度是否零税（对标 aclnn 5.86ms/matmul）。**已打通**：shim（graph API 构图 → aclgrphBuildModel 内存编译 → aclmdlExecuteAsync 绑定自有 device buffer）+ standalone main（verify/bench 双模式）编译链；**硬知识**：CANN libgraph_base 是 pre-cxx11 string ABI（`_GLIBCXX_USE_CXX11_ABI=0` 必须）；GE 构图只能在 **apxinf_npu 容器**跑（TeFusion 算子编译器走 python/tbe 栈，rust 容器 py_decouple 起不来，PYTHONPATH 救不了；npu 容器需 LD_LIBRARY_PATH=ge_poc/build+CANN lib64 + PYTHONPATH=CANN site-packages）；算子必须 `OperatorFactory::CreateOperator`（裸 Operator(name,type) 是无 IR 空壳，UpdateOutputDesc/SetInput 全失败）；MatMulV2 输入 x1/x2 输出 y 属性 transpose_x1/x2 有效。**当前卡点（下轮第一动作）**：`Graph::SetInputs` 必须在 AddNodeByOp **之前**（graph.cc:56：AddNodeByOp 会 init 内图锁死 SetInputs）；operator 级 SetInput 连接在 AddNodeByOp 后不保留（报 "mm1_0 input 0 not linked"）→ 改造为 SetInputs 先行 → AddNodeByOp → GetAllNodes/FindNodeByName 拿 GNode → **GNode 级 AddDataEdge 连边**（当初 AddDataEdge 失败疑因也是 SetInputs 未先调）→ SetOutputs（GNode pair 版）。之后 verify 模式过对拍 → bench 模式拿零税判定 → 决定 C 路线全量投入与否
