...
- [ ] **下一章：PI0.5 executor（M2 运行半 + M3 的主体）——结构侦察完成（含一次认知修正，2026-09-17）**：
  ...
  - **镜像施工图（2026-09-17 终版，零探索成本可开工）**——依赖链 `vla_runtime(1013) → bf16_runtime(690) → bf16_executor(228) → kernels`，全部新增文件不动 cuda：
    ...
    - **⚠ C 阶段核心卡点——大矩阵 matmul 在 310P3 的可行路径（2026-09-18 排查现场）**：
      ...
      - **优化待办（性能阶段，目标对标 378ms torch_npu 基线，当前 1.93s = 5.1×）**：① ada-norm 的 host_f16_row 每次读 style d2h+sync（18 层 ×3 × 10 步 ≈ 540 次 stream 打断——大头，style 预解析成设备行可消除）② ACLGraph 捕获（ada-norm host 读需先重构，见 ascend_runtime.rs 头注）③ eager 逐 op 两段式 launch 开销（可批处理/缓存 workspace）④ rope 组合实现可换单算子（V2 变体）⑤ kv_bias host 往返 → 加载期预切
