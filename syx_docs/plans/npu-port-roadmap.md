  ...
  - **镜像施工图（2026-09-17 终版，零探索成本可开工）**——依赖链 `vla_runtime(1013) → bf16_runtime(690) → bf16_executor(228) → kernels`，全部新增文件不动 cuda：
    ...
    - **⚠ C 阶段核心卡点——大矩阵 matmul 在 310P3 的可行路径（2026-09-18 排查现场）**：
      ...
      - **✅ 性能阶段推进（2026-09-19 凌晨）：全深度 906.9 → 680.8ms**。三项落地：
        ...
        - **下一波（数据驱动，预计 → ~500ms）**：① ada-norm mul+add → `aclnnAddcmul`（头文件已确认存在；注意 host aclScalar 捕获风险 107030 先例）② gelu+mul → `aclnnGeGlu(self, dim, approximate, out, outGelu)`（单输入自拆半，正中 gate_up 形态；activateLeft 语义对拍判别）③ 之后视情况 AscendC fused ada-norm（3→1，skill 已备）
