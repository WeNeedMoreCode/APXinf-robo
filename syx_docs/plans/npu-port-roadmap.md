  ...
  - **镜像施工图（2026-09-17 终版，零探索成本可开工）**——依赖链 `vla_runtime(1013) → bf16_runtime(690) → bf16_executor(228) → kernels`，全部新增文件不动 cuda：
    ...
    - **⚠ C 阶段核心卡点——大矩阵 matmul 在 310P3 的可行路径（2026-09-18 排查现场）**：
      ...
      - **✅ 性能阶段推进（2026-09-19 凌晨）：全深度 906.9 → 680.8ms**。三项落地：
        ...
        - **剩余唯一大项：~290ms 空隙的来源**（680ms 墙钟 - 392ms kernel 执行；**三次实验证明它不随 op 数下降**——广播缓存删 800 kernel 只省 2ms）。候选：AIC↔AIV 切换开销 / task 间调度串行 / matmul 依赖链等待。取证工具：msprof 的 step_trace.db / hwts 时间线（看 kernel 间空隙形态——固定 per-task 还是集中在某些 op 类型周围）。**若空隙不可消除 → 680ms 接近此架构上限，验收线（491ms）不可达 → 进决策会谈**；若可归因（如某类 op 的等待）→ 定向消除
