  ...
  - **镜像施工图（2026-09-17 终版，零探索成本可开工）**——依赖链 `vla_runtime(1013) → bf16_runtime(690) → bf16_executor(228) → kernels`，全部新增文件不动 cuda：
    ...
    - **⚠ C 阶段核心卡点——大矩阵 matmul 在 310P3 的可行路径（2026-09-18 排查现场）**：
      ...
      - **✅ 性能阶段推进（2026-09-19 凌晨）：全深度 906.9 → 680.8ms**。三项落地：
        ...
        - **⭐⭐ 性能验收线判定（2026-09-19，提交用户决策）**：当前 679.8ms（全深度单图 replay，对拍 0.019）。aclnn+ACLGraph 架构内剩余手段：qkv 三合一（-396 matmul task，净 ~-90ms 扣 take_rows 税 → ~590ms）。**验收线 491ms（378×1.3）在此路径不可达**（差 ~20%）。根本差距 = ACLGraph task 启动税（~400ms/链）vs GE 静态 OM 零空隙编排——torch_npu 路线的本质优势。选项：**A** 接受 ~590ms（qkv 融合后）"部署形态优先"进 M3（LIBERO 对标，边缘单进程 Rust 引擎 vs python+torch 全家桶）；**B** 310P3 定位验证芯片、性能目标移至 910 系/下一代（GE 原生图路线是另一量级工程）；**C** 追 GE 原生 OM 路线（本地 ge 仓在 D:/compass/ge，工程量数周级）
