...
- [ ] **下一章：PI0.5 executor（M2 运行半 + M3 的主体）——结构侦察完成（含一次认知修正，2026-09-17）**：
  ...
  - **镜像施工图（2026-09-17 终版，零探索成本可开工）**——依赖链 `vla_runtime(1013) → bf16_runtime(690) → bf16_executor(228) → kernels`，全部新增文件不动 cuda：
    ...
    - **施工顺序**：A. ~~补 ops~~ ✓ → B. ~~executor 层函数镜像~~ **✓ 完成（3e05c7a，0 错误）**：language/vision/action/patch_embed + AscendRopeCache（Arc 表）+ `aq::` 错误桥 + ada-norm（ones-gamma + row_replicate (1+style)）+ 前缀 KV cat + BSH PFA 零转置 → C. `pi05/ascend_runtime.rs`（690 行镜像：权重走 bf16_weights+to_device、prefix KV、denoise 循环、CapturedGraph 用 aclmdlRI）→ D. `pi05/ascend_vla_runtime.rs`（1013 行：tuning 桩化、patchify CPU）→ E. mod.rs ascend 门 + load 注册 → F. random-weights bench = **M2 运行半**。⚠ B 未做数值验证（需真权重，C 串通后对拍）
