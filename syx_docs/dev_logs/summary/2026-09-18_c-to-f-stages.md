...
6. **下一波**：~~aclnnAddcmul + aclnnGeGlu~~ **双双证伪（同日）**——Addcmul torch_npu 可用（对拍 0.005）但实测无收益（684.4 vs 680.8ms）+ fp16 累加顺序漂移对拍 0.048 贴断言线 → 撤回；GeGlu 两段式调用**挂死**（10 分钟不返回，kill 取证；torch_npu 无分发、GE 从未融合——三重证据）→ 撤回。**AscendC fused ada-norm（5→1）已建成（98aabfc）**：完整工具链全绿（cmake 编译/dlopen/对拍 0.004/ACLGraph 捕获 replay 对拍 0.019），但 **replay 打平（679.8 vs 681.6ms）**——三次实验收敛同一结论：图内设备时间由带宽决定，kernel 数不是成本。取证战果：硬件 ReduceSum/RmsNorm 被 arch 门控挡在 310P3 外（torch_npu 无融合 rms 的根因）；跨行软件流水线在 kernel 内复现 + GetValue 双 pin 修复。
7. **空隙来源闭环（a7cdde5）**：msprof timeline gap 分析——链内空隙 ~401ms，**81% 在 MatMulV2 task 前（334µs × 970 次/链）**；元素 op 启动 3-6µs。ACLGraph matmul task 启动税坐实。换入口判死（Mm/Gemm/MatMulCommon 图内三档同分）。**⭐⭐ 验收线判定触发：491ms 在 aclnn+ACLGraph 路径不可达（qkv 融合后 ~590ms 封顶），提交用户决策（选项 A/B/C 见 roadmap）**——根本差距是 GE 静态 OM 编排（torch 路线零启动税、matmul task 批量组织 105/iter vs 我们 970）
8. **用户决策 → C 路线（GE 原生 OM）POC 开工（1ed06e2）→ ✅ 同日收官（28adf78）**：用户质疑"不应该比 python 慢"驱动三轮插桩后选 C。POC 五连修复（全取证定位）：① AddNodeByOp 与 SetInputs 是互斥构图路径——纯 operator 流物化整图；② MatMulV2 infershape rank 门槛 → 经典 MatMul 绕过；③ build_options 缺 input_shape → 9KB 空 model；④ Data 幽灵 input desc 决定 IO dtype → 插 Cast 全零；⑤ **rust 容器（9.0.1）GE 起不来的根因 = 缺 python3-config**（strace：`python3-config --prefix` 退 127 → py_decouple 死）——一个符号链接修复，npu 容器不再必需（用户推动重查 9.0.1，判断正确）。**终局数据**：verify 0.002075；bench m=832 GE OM 4.745 vs aclnn 5.812 ms/matmul（−18%，23.5 TFLOPS），拟合固定税 555µs/task→0、斜率 −20%（静态 tiling 更优）。**C 路线立项：全量施工开工**（施工图见 roadmap）
