...
8. **用户决策 → C 路线（GE 原生 OM）POC 开工（1ed06e2，进行中）**：用户质疑"不应该比 python 慢"驱动三轮插桩（torch 同口径空隙 17.3ms/iter、无 profiler 拟合 475µs/task 税、qkv 已融合无空间）后选 C。GE 是纯 C++ 库（torch 只是前端）；PI-0.5 算子面（MatMulV2/LayerNormV4/RmsNorm/PFA/ApplyRotaryPosEmb/GeluV2）内置全在。POC=16 个 MatMulV2 串联成 OM 实测零税。已通：编译链（`_GLIBCXX_USE_CXX11_ABI=0`——CANN 旧 string ABI）、GE init（**仅 npu 容器**：TeFusion 走 python/tbe 栈）、工厂算子（裸 Operator 无 IR）。卡点：SetInputs 须在 AddNodeByOp 前 + GNode 级 AddDataEdge 连边（下轮第一动作，详见 roadmap C 路线节）
