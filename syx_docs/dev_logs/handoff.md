...
继续 APXinf 昇腾 NPU（**✅ python 宿主 GE 障碍终结 2026-09-24（子模块 2dbffe9）：gdb 栈定罪 = te fusion py_decouple static path 的 `TE_PyEval_SaveThread()` 偷走 PyO3 持有的 GIL 并存走 thread state → open 返回时 PyO3 into_py 在烂解释器状态上 pymalloc SEGV（obmalloc.c get_state）；修复 = GeServeModel::open 主体包 `Python::allow_threads`（无 GIL 进入 ⇒ te 不触发 SaveThread）+ ge_init_once 默认 `MIN_COMPILE_RESOURCE_USAGE_CTRL=ub_fusion,op_compile`（forkserver off——中途静默死必要条件）；验证 open 176.4s 完整返回 + infer×3 max_diff=0.0244 rel=1.0% 与 spool 桶逐位同 + 250-262ms/call + PYO3_GESERVE_OK；inproc 全链 rust 容器内无障碍**——见 summary/2026-09-24_pyhost-ge-survival-hunt.md）。下一轮按优先级：**A3. eval 全链 inproc（⚠ 2026-09-24 上午重估：性价比降级为可选——实测 inproc 收益仅 ~4-6ms/帧（gatefastall 256.3 vs 直调 250-252，非早期估的 15ms），而成本遇 abi 墙：npu 容器 python 3.11.10 vs cdylib 编译于 rust 容器 python 3.12.13（PyO3 扩展 abi 绑定）——要么 npu 容器统一 3.12（重装 torch/lerobot 全套）要么 rust 容器补 3.11 dev 重建 cdylib；外加 torch_npu 剥离 = 轻量预处理宿主重写（ModelZoo patch 在 from_pretrained 里硬编码 .to("npu")+FRACTAL_NZ，CPU 化不能只改 device 参数——需要只要 pre/post processor+config 不要 7GB 权重的新宿主类）。若做：① 9.0.1 libs 拷 /data（依赖面已勘：libascendcl/opapi/nnopbase/msprofiler/ge_common_base/exe_graph/register/libgraph 全在 toolkit lib64 + ge_builder.so；lib64 是多层 symlink，du 需 readlink -f 后再算）② 3.12 环境统一 ③ 轻量宿主 + 对拍 ④ task0 验证）**；B. 动态 L 定形（预置桶集 tl138-148+tl200 已备；注意 inproc 单管线 env 全局——多桶须多进程或扩展；**优先级因 A3 降级而升为首选**）；C. E msprof vision 65ms（10-15ms 级收益 > inproc 5ms，同为首选）/ D int8 probe 顺路。⚠ 库面纪律照旧：GeServe::open env 钉死不含 GEB_DEPTH；稳定复验脚本 pyo3_stability.sh 在 /data/apxinf/pyo3_check/。验证梯照旧（golden ≤0.1%/≤2% → 冒烟 A/B bit 同 → task0 → 全量大改后）。
