...
继续 APXinf 昇腾 NPU（**python 宿主 GE 障碍破一半 2026-09-24：死因链 = aclgrphBuildInitialize→TbeInitialize→PythonAdapterManager——no-init A/B 进程不死（构图 panic 证明构图依赖 FE init）；`MIN_COMPILE_RESOURCE_USAGE_CTRL=ub_fusion,op_compile`（te 侧 env 直读）关 forkserver 并行编译后 open 三段全通、死亡点推进到 open 尾后显式 SEGV RC=139 带 core 可 gdb；python 宿主天然走 py_decouple static path（dlsym 命中宿主符号→操纵宿主解释器+GIL），rust 宿主走 dynamic path（dlopen 独立 libpython）——spool 稳定根源；子模块 c883290：ge_init_once 默认跳 init + 逃生门 APXINF_GE_BUILD_INIT=1**——见 summary/2026-09-24_pyhost-ge-survival-hunt.md）。下一轮按优先级：**A1. gdb core dump 定剩余死因（容器 core pattern 确认落点；嫌疑 = InitPyModuleAndApiCall te 模块 import 注入宿主解释器 / HandleManager GIL / GE 收尾）**；**A2. 构图与 python 宿主剥离工程**（"init 但不碰 python"选项面不存在——InitPyModuleAndApiCall 无条件；候选 a. fork worker 子进程做 init+构图+open+pipe 通信，b. binds 与构图解耦从 OM introspection 来（geb_model_load 已 CacheIo）→ init 整个不需要）；A3. eval 全链 inproc（9.0.1 libs 拷 /data 供 npu 容器 + torch_npu 剥离=前处理 CPU 化，9.0.1 toolkit 在 rust 容器镜像层非挂载）；B. 动态 L 定形（预置桶集 tl138-148+tl200 已备）；C. E msprof vision 65ms / D int8 probe 顺路。⚠ 库面纪律照旧：GeServe::open env 钉死不含 GEB_DEPTH；pyo3_noinit.sh 判决脚本在 /data/apxinf/pyo3_check/（当前形态=逃生门+forkserver 关）。验证梯照旧（golden ≤0.1%/≤2% → 冒烟 A/B bit 同 → task0 → 全量大改后）。
...
| **C** | ✅ **已落地（2026-09-23，e7e4cd1+787730a+engine.py inproc 分支）；python 宿主障碍破一半（2026-09-24 c883290，见 summary/2026-09-24_pyhost-ge-survival-hunt.md）**：**in-process serving**——执行器沉 crate 库面（GeServe 门面，spool/直调同源帧实现）+ PyO3 GeServeModel + npu_ge.py `APXINF_GE_TRANSPORT=inproc`（回落 spool 保底）。障碍现状：forkserver 并行编译（中途死必要条件）已可关（`MIN_COMPILE_RESOURCE_USAGE_CTRL=ub_fusion,op_compile`），剩余 = open 尾后 SEGV 带 core（gdb 待做）；**构图依赖 init ⇒ 出路是构图/python 宿主剥离（fork worker 或 binds 从 OM introspection 来）** | spool 15ms 在 inproc 生效环境免 | 引擎单实现三入口（example/PyO3/spool） | 已过：golden+bit 恒等双验、rust 容器 python 三段直调全通；task0/全量回归走 spool 保底无回归 |
...
**建议路线**：~~A+B 先做~~ **已完成（全量口径 256.3ms = 0.682×，10/10）**。~~主线 = 引擎接入~~ **三步已落地（①库化 e7e4cd1 ②PyO3 787730a ③engine.py inproc 分支）**。下一轮优先级：**python 宿主障碍后半场**（gdb core 定剩余死因 → 构图/宿主剥离工程 → eval 全链 inproc：9.0.1 libs 供给 + torch_npu 剥离）→ ④ 动态 L 定形；D int8 probe / E msprof 顺路。
